"""Strategy 1 test on CSI300 — does China cross-asset / vol data improve the
LINEAR SIGN model (the hybrid's directional head)? (PROJECT_NOTES 7.18,
CSI300 adaptation)

Compares three L2-logistic sign models on the SAME CSI300 TACR magnitudes:
  A_full : 8 CSI300 z-features, trained on the full train split (reference).
  A_match: 8 CSI300 z-features, trained ONLY where the macro set is available
           (>= 2015-02, QVIX inception) — isolates date-matching.
  B      : 8 CSI300 z-features + 6 CSI300 macro features (causal z), same
           matched dates — isolates the macro contribution.

Discipline identical to the SPY version: L2 LogisticRegression, C on VAL from
{0.01,0.1,1,10}, TEST untouched. Margins vs the hybrid's own EM control
(|a_TACR|*m). 5-seed pack = CSI300 TACR per-seed checkpoints.

Run: python scripts/hybrid_sign_macro_cn.py
"""
...
CHECKPOINTS = ROOT / "src" / "models" / "tacr" / "checkpoints"
CSI_Z = ["z_ret_1d", "z_ret_5d", "z_ret_20d", "z_realized_vol_20d",
...
def main() -> None:
    ...
    csi_z = feats[CSI_Z].reindex(data.dates).to_numpy(dtype="float64")     # (T, 8)
    ...
    macro_z = macro_z[np.searchsorted(macro_raw.index, naive)]             # (T, 6)
    Fb = np.concatenate([csi_z, macro_z], axis=1)                          # (T, 14)
    ...
    ok_z = np.isfinite(csi_z).all(1)
    ok_mac = np.isfinite(Fb).all(1)

    models = {
        "A_full": fit(csi_z[tr_mask & ok_z], y[tr_mask & ok_z], csi_z[va_mask & ok_z], y[va_mask & ok_z]),
        "A_match": fit(csi_z[tr_mask & ok_mac], y[tr_mask & ok_mac], csi_z[va_mask & ok_mac], y[va_mask & ok_mac]),
    ...
    for name, Xm in [("A_full", csi_z), ("A_match", csi_z), ("B", Fb)]:
        m, C = models[name]
        tr_n = int((tr_mask & (ok_mac if name != "A_full" else ok_z)).sum())
