"""Exp 2 - CSI300 TRANSFER of the frozen canonical C+ protocol. PROJECT_NOTES 7.37.

Transfers the FROZEN CANONICAL protocol (7.33/7.36) from TRUE SPY to CSI300:
...
2019-2020, test 2021-2024), same single-logistic P(up) head C-selected on val,
same sizing rules vs own |a| EM (scale-invariant margin), same cost grid, same
10-RNG-seed resampled-P robustness sweep.

The protocol runs on the CSI300 Phase-1 artifacts in
data/processed_csi300_backup/ (naive daily index, macro features recomputed on
the CSI300 close = "instrument ret vs US cross-assets"). This is a faithful
re-estimation test: does the SPY-null canonical sign + the |2P-1| sizing edge
replicate on a second market, reported honestly?

WARNING/ASSUMPTION: the OOD arm at the end applies the SPY-trained canonical P
directly to CSI300 z-features WITHOUT retraining -- a genuine out-of-distribution
cross-market probe. It is diagnostic only: causal z-statistics are market-local,
so equalizer scales differ. Interpret P_up relative to 0.5, not as probabilities.

Artifacts: data/csi300_transfer.csv (per-day test table) + console report.

Run: python scripts/csi300_transfer.py
"""
...
CSI_DIR = ROOT / "data" / "processed_csi300_backup"
...
def main() -> None:
    print("=" * 78)
    print("EXP 2 - CSI300 TRANSFER of frozen canonical C+ protocol")
    print("=" * 78)

    # ---- CSI300 (primary): re-estimate the frozen protocol on CSI300 data ---
    dates, Fb, mkt, y = build_frame(CSI_DIR / "features_regimes.parquet")
    ...
    print(f"\nCSI300: dates {dates.min().date()}..{dates.max().date()} "
    ...
    print(f"  corr(P[t], mkt[t])   = {float(np.corrcoef(P, mkte)[0,1]):+.4f} "
          f"(predictive if ~0) | corr(P[t], mkt[t+1])=N/A (overlap)")
    drift_log = float((np.log1p(np.abs(mkte)) > 0).mean())  # placeholder clarity
    ...
    tbl.to_csv(ROOT / "data" / "csi300_transfer.csv", index=False)
    ...
    # ---- robustness sweep: same protocol, 10 resampled P heads --------------
    print("\n" + "=" * 78)
    print("ROBUSTNESS: 10 RNG seeds, resampled-bootstrap P heads (CSI300)")
    print("=" * 78)
    ...
    # ---- OOS window check: SAME frozen head, untouched 2025-2026 ------------
    print("\n" + "=" * 78)
    print("OOS WINDOW CHECK: same CSI300 head, untouched 2025-2026 (no retrain)")
    print("=" * 78)
    ...
    # ---- OOD arm (diagnostic): SPY-trained P applied to CSI300 z-features ----
    print("\n" + "=" * 78)
    print("OOD DIAGNOSTIC: SPY-trained canonical P -> CSI300 features (no retrain)")
    print("=" * 78)
    ...
    # CSI300 test features, same 16-dim space (recomputed CSI300 frame)
    ...
    print(f"  acc(SPY model on CSI300 test) = {acc_o:.4f} (up-rate {float((yte>0).mean()):.4f})")
    print(f"  sign-only margin(1bp) = {sp_ - e_:+.4f}   |2P-1| margin(1bp) = {sp2 - e2:+.4f}")
    print(f"  corr(P_ood, P_csi300) = {np.corrcoef(P_ood, P)[0,1]:+.4f} "
          f"(1.0 would mean identical predictions)")
    print(f"\nsaved per-day table -> data/csi300_transfer.csv")
